...
bool tImageKTX::Set(tPixel4b* pixels, int width, int height, bool steal)
{
	Clear();
...
int tImageKTX::StealCubemapLayers(tList<tLayer> layerLists[tFaceIndex_NumFaces], uint32 faceFlags)
{
...
int tImageKTX::GetCubemapLayers(tList<tLayer> layerLists[tFaceIndex_NumFaces], uint32 faceFlags) const
{
	if (!IsValid() || !IsCubemap() || !faceFlags)
		return 0;
